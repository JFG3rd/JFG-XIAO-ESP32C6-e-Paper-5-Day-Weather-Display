...
// LOCAL PATCH (not upstream Seeed_GFX): the original CHECK_BUSY() spun forever
// on `while (!digitalRead(TFT_BUSY));`. If the panel never raises BUSY - which
// happens when it is left in DSLP and the MCU deep-sleeps with the control pins
// floating - epaper.begin() never returns, so Wi-Fi and the web server never
// start and the device is unreachable and undebuggable. Bounding the wait makes
// a sick panel a degraded feature instead of a bricked board.
//
// epdBusyTimedOut is defined by the application; it latches true so the sketch
// can tell whether the panel actually responded. See docs/EPAPER_2IN9_BWRY_XIAO_GUIDE.md.
#ifdef TFT_BUSY
extern volatile bool epdBusyTimedOut;
// Runtime-adjustable so the sketch can wait out a full refresh when the BUSY
// line itself is suspected broken (see handlePanelForce in the application).
extern volatile uint32_t epdBusyTimeoutMs;
#define CHECK_BUSY()                                              \
    do                                                            \
    {                                                             \
        uint32_t _busyStart = millis();                           \
        while (!digitalRead(TFT_BUSY))                            \
        {                                                         \
            if ((millis() - _busyStart) > epdBusyTimeoutMs)       \
            {                                                     \
                epdBusyTimedOut = true;                           \
                break;                                            \
            }                                                     \
            delay(1);                                             \
        }                                                         \
